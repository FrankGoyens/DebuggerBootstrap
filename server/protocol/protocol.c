...
void MakeProjectDescriptionPacket(struct ProjectDescription* description, char** packet, size_t* packet_size) {
    ...
    *packet_size = 3u * sizeof(uint8_t) + json_length;

    *packet = (char*)malloc(*packet_size);
    char* packet_content = *packet;
    packet_content[0] = version;
    packet_content[1] = lower_json_length_byte;
    packet_content[2] = upper_json_length_byte;
    memcpy(&packet_content[3], project_description_json_string, json_length);
